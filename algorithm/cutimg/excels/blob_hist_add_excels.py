...
def myblobhist():
    ...
    sheet1 = f.add_sheet('斑块统计特征', cell_overwrite_ok=True)

    row0 = ["c/nc", "v/i", "h", "面积", "周长", "圆形度", "矩形度", "伸长度"]
    ...
    # cv2.waitKey(0)
    sheet1.write_merge(0, 0, 0, 0, 'images_camouflage')
    sheet1.write_merge(0, 0, 0, 0, 'mix')
    sheet1.write_merge(0, 0, 0, 0, '20m')
    sheet1.write_merge(0, 0, 0, 0, arr[0])
    sheet1.write_merge(0, 0, 0, 0, arr[1])
    sheet1.write_merge(0, 0, 0, 0, arr[2])
    sheet1.write_merge(0, 0, 0, 0, arr[3])
    sheet1.write_merge(0, 0, 0, 0, arr[4])
    f.save(path_excels_save + '/' + 'excel_blob_hist.xls')
